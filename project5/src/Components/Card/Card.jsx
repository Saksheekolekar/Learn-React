  ...
  return (
    <div className={`flex flex-col items-center p-4 dark:bg-slate-700 bg-white shadow-md rounded-lg transform transition-transform ${isActive ? 'scale-105 dark:shadow-slate-200 shadow-black shadow-lg' : 'scale-100'}`}>
      ...
      <div className="mt-4 text-center">
        <p className="text-lg dark:text-slate-50 font-semibold">{name}</p>
        <p className=" font-sans dark:text-zinc-500">{description}</p>
        <p className="mt-2 font-mono dark:text-gray-400 hover:text-gray-900 hover:underline text-gray-600">{technologies.join(' || ')}</p>
        <a
          href={link}
          target="_blank"
